...
const RecipeBookPage = ({ userId }) => {
  ...
  const [recipes, setRecipes] = useState([
    {
      recipe_name: "Waakye",
      description: "Waakye is a beloved Ghanaian dish known for its distinctive red color and rich flavor. This recipe combines rice and black-eyed beans with waakye leaves, creating a delightful and nutritious meal that is cherished in Ghanaian cuisine.",
      prep_time: "1 hour 15 minutes",
      difficulty: "Hard",
      servings: "6",
      instructions: ['Rinse the rice and millet (if using) under cold water until the water runs clear. Drain and set aside.', 
      ...
      ingredients: ["2 cups rice (preferably a mix of white rice and millet)",
    ...
    description: "Green Chile Chicken Enchiladas are a delectable Mexican dish featuring tender taco-seasoned shredded chicken and a flavorful green chile enchilada sauce, all wrapped in soft tortillas and topped with a blend of creamy mozzarella and Jack cheese. These enchiladas offer a perfect balance of spice and cheese, making them a delightful treat for any occasion.", 
    ...
    ingredients: ["2 cups cooked and shredded chicken (seasoned with taco seasoning)",
  ...
  const handleSubmitRecipe = async (recipe_name) => {
    try {
            ...
            displayedRecipes.map((recipe) => (
              <Grid item xs={12} sm={6} md={4} key={recipe.recipe_name}>
                ...
                >
                  ...
                  <Button onClick={() => handleDeleteRecipe(recipe.recipe_name)} variant="contained" sx={{mt: 3, backgroundColor: "white", color: 'black'}}>x</Button>
                </Card>
              </Grid>
            ))
          ) : (
